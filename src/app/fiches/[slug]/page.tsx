...
import { getAllFiches, getFicheBySlug, getAllContacts } from "@/lib/db";
...
export default async function FicheDetailPage({ params }: Props) {
  ...
  const fiche = await getFicheBySlug(slug);
  if (!fiche) notFound();
              ...
              >
                <div
                  className={`px-4 py-3 flex items-start gap-3 ${
                    etape.critique ? "border-b border-red-200" : ""
                  }`}
                >
                  <div
                        ...
                        <li key={i} className="flex items-start gap-2">
                          <CheckCircle
                            ...
                            className={`flex-shrink-0 mt-0.5 ${
                              etape.critique ? "text-red-500" : "text-blue-500"
                            }`}
                          />
